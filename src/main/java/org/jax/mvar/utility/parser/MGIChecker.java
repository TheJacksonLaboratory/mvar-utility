...
public class MGIChecker {
    ...
    public void loadVCF(File vcfFile) {
        ...
        try (Connection connection = DriverManager.getConnection(config.getUrl(), config.getUser(), config.getPassword())) {
            ...
            Map<String, Variant> variations = VcfParser.parseVcf(connection, vcfFile, vcfFile, false, false);
    ...
    private Map<Integer, Variant> queryDatabase(Connection connection, Map<String, Variant> variations) throws SQLException {
        ...
        for (String key : variations.keySet()) {
            ...
            try (PreparedStatement selectCanonicalIdStmt = connection.prepareStatement(selectCanonical)) {
    ...
    private void writeToFile(Map<Integer, Variant> results) {
        ...
        try {
            ...
            br.write("id\tvariant_ref_txt" + System.lineSeparator());
            for (Integer key : results.keySet()) {
                Variant var = results.get(key);
                br.write(key + "\t" + var.getVariantRefTxt() + System.lineSeparator());
            }
        } catch (IOException e) {
            System.err.println("Error writing MGI duplicates to file: " + e);
        } finally {
            try {
                if (br != null)
                    br.close();
